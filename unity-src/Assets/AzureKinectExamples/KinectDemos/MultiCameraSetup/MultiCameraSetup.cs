...
namespace com.rfilkov.components
{
    ...
    public class MultiCameraSetup : MonoBehaviour
    {
        ...
        private static int[] trackedBodyJoint =
        {
            ...
            (int)KinectInterop.JointType.SpineNaval, (int)KinectInterop.JointType.SpineChest, (int)KinectInterop.JointType.Neck,
            (int)KinectInterop.JointType.Head, (int)KinectInterop.JointType.EyeLeft, (int)KinectInterop.JointType.EyeRight,
            ...
            (int)KinectInterop.JointType.WristLeft, (int)KinectInterop.JointType.WristRight,
            ...
            (int)KinectInterop.JointType.AnkleLeft, (int)KinectInterop.JointType.AnkleRight
        };
        ...
        private void SaveSensorPose(int sIndex, Vector3 camPos, Quaternion camRot, ref UserBodyPose userPose)
        {
            ...
            if (savedCamPos[sIndex].Count == savedCamRot[sIndex].Count)
            {
                ...
                int cpi = GetBestCamPoseIndex(sIndex, fromSavedPR, numSavedPR, ref minPoseError[sIndex]);
                if(cpi >= 0)
                {
        ...
        private int GetBestCamPoseIndex(int sIndex, int fromPose, int toPose, ref float minPoseError)
        {
            if (sIndex == 0)
            ...
            for (int c = 0; c < savedCamPos[sIndex].Count; c++)
            {
                ...
                for (int i = fromPose; i < toPose; i++)
                {
                    ...
                    for (int j = 0; j < numJoints; j++)
                    {
                        ...
                        fPosError += Vector3.Distance(posRefUserJ, posCamUserJ);
                ...
                if(numProcPoses > 0)
                {
                    fPosError /= numProcPoses;  // independent of numPoses
                    fPosError *= (float)(toPose - fromPose + 1) / numProcPoses;  // respect the all-poses/proc-ones ratio

                    if (fPosError < fErrorMin)
                    {
                        fErrorMin = fPosError;
                        iPoseIndexMin = c;
                        //Debug.Log("S" + sIndex + " PoseIndexMin: " + iPoseIndexMin + ", errMin: " + fErrorMin + ", procPoses: " + numProcPoses + ": from: " + fromPose + ", to: " + toPose);
                    }
                }
            }
